"""Sync GraphQL HTTP client.

Hand-edit ``turbine/python-sdk/codegen/base_client.py``. ariadne-codegen
...
from .exceptions import (
    REASON_MISSING_DATA_AND_ERRORS,
    REASON_NOT_JSON,
    GraphQLClientGraphQLMultiError,
    GraphQLClientHttpError,
    GraphQLClientInvalidResponseError,
    operation_name_from_response,
)
...
class BaseClient:
    ...
    def get_data(self, response: httpx.Response) -> dict[str, Any]:
        ...
        try:
            ...
            raise GraphQLClientInvalidResponseError(
                response=response, reason=REASON_NOT_JSON
            ) from exc
        ...
        if (not isinstance(response_json, dict)) or (
            ...
            raise GraphQLClientInvalidResponseError(
                response=response, reason=REASON_MISSING_DATA_AND_ERRORS
            )
        ...
        if errors:
            raise GraphQLClientGraphQLMultiError.from_errors_dicts(
                errors_dicts=errors,
                data=data,
                operation_name=operation_name_from_response(response),
            )
