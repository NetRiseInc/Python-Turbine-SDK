...
`sdk.iter_asset_group_members(...)` paginates for you and accepts filter kwargs.
...
def main() -> None:
    ...
    for item in sdk.iter_asset_group_members(asset_id="asset_123", page_size=10, max_pages=1):
        print(item)
...
Same data via `client.query_asset_group_members` when you need exact GraphQL control.
