...
`sdk.iter_public_keys(...)` paginates for you and accepts filter kwargs.
...
def main() -> None:
    ...
    for item in sdk.iter_public_keys(asset_id="asset_123", page_size=10, max_pages=1):
        print(item)
...
Same data via `client.query_public_keys` when you need exact GraphQL control.
