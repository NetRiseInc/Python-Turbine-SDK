...
`sdk.iter_assets_relay(...)` paginates for you and accepts filter kwargs.
...
def main() -> None:
    ...
    for item in sdk.iter_assets_relay(page_size=10, max_pages=1):
...
Same data via `client.query_assets_relay` when you need exact GraphQL control.
