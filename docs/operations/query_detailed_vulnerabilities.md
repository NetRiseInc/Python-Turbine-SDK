...
`sdk.iter_detailed_vulnerabilities(...)` paginates for you and accepts filter kwargs.
...
def main() -> None:
    ...
    for item in sdk.iter_detailed_vulnerabilities(asset_id="asset_123", page_size=10, max_pages=1):
        print(item)
...
Same data via `client.query_detailed_vulnerabilities` when you need exact GraphQL control.
